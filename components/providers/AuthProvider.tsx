...
export function AuthProvider({ children, initialUser }: AuthProviderProps) {
	...
	const checkSession = useCallback(async () => {
		if (!backendUrl) {
			console.error(
				"Frontend: Backend URL is not configured for checkSession.",
			);
			setUser(null);
			setIsLoading(false);
			return;
		...
		try {
			...
			if (response.ok) {
				...
				setUser(userData);
				console.log(
					"Session check (client): User data fetched",
					userData.email,
				);
			} else {
				setUser(null);
				if (response.status === 401) {
					console.log(
						"Session check (client): No active session or token expired.",
					);
				} else {
					console.error(
						"Session check (client): Failed to fetch profile:",
						response.status,
						await response.text(),
					);
				}
	...
	return (
		<AuthContext.Provider
			value={{ user, isLoading, login, logout, checkSession }}
		>
			{children}
		</AuthContext.Provider>
	);
